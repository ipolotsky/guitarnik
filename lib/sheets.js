...
async function readSheet(key, options) {
  const table = await fetchTable(key);
  const settings = options == null ? {} : options;
  if (settings.requireHeaders === true) {
    const missing = reference.SHEET_HEADERS[key].filter(x => table.headers.indexOf(x) < 0);
    if (missing.length > 0) {
      throw new google.SheetsError(
        `В листе «${table.name}» не хватает колонок: ${missing.join(', ')}. Поправь заголовки в таблице и повтори`,
      );
    }
  }
...
async function deleteRows(key, ids) {
  if (ids.length === 0) {
    return { deleted: 0 };
  }
  const table = await fetchTable(key);
  ...
  const numbers = ids
    .map(x => findRowNumber(table.values, idIndex, x))
    .filter(x => x > 0)
    .sort((a, b) => b - a);
  if (numbers.length === 0) {
    return { deleted: 0 };
  }
  const sheetId = await findSheetId(table.name);
  if (sheetId == null) {
    return { deleted: 0 };
  }
  const requests = numbers.map(number => ({
    deleteDimension: {
      range: { sheetId, dimension: 'ROWS', startIndex: number - 1, endIndex: number },
    },
  }));
  await call(table.name, client => client.spreadsheets.batchUpdate({
    spreadsheetId: spreadsheetId(),
...
async function findSheetId(name) {
  const response = await call(name, client => client.spreadsheets.get({
    spreadsheetId: spreadsheetId(),
  ...
  const sheets = response != null && response.data != null && Array.isArray(response.data.sheets)
    ...
    : [];
  const found = sheets.find(x => x != null && x.properties != null && String(x.properties.title) === name);
  return found == null ? null : found.properties.sheetId;
}
...
module.exports = { readSheet, syncRows, replaceSheet, deleteRows };
