...
function addMissingColumns(db, key) {
  const existing = db.prepare(`PRAGMA table_info(${key})`).all().map(x => x.name);
  reference.SHEET_HEADERS[key].concat(['position']).forEach(column => {
    if (existing.indexOf(column) >= 0) {
      return;
    }
    const definition = column === 'position' ? 'INTEGER' : "TEXT NOT NULL DEFAULT ''";
    db.exec(`ALTER TABLE ${key} ADD COLUMN ${column} ${definition}`);
  });
}
