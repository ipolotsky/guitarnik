...
async function createStructure() {
  ...
  for (const key of KEYS) {
    const name = current.sheets[key];
    const values = await readValues(spreadsheetId, name);
    if (matchesHeaders(key, values[0])) {
      continue;
    }
    if (!isFirstRowEmpty(values) && !looksLikeHeaders(values[0]) && sheetIds[name] != null) {
      await insertTopRow(spreadsheetId, sheetIds[name]);
    }
...
async function insertTopRow(spreadsheetId, sheetId) {
  const request = {
    insertDimension: {
  ...
  await addSheets(spreadsheetId, [request]);
}
...
async function readSheetIds(spreadsheetId) {
  ...
  const result = {};
  sheets.forEach(x => {
    if (x != null && x.properties != null && x.properties.title != null) {
      result[String(x.properties.title)] = x.properties.sheetId;
    }
  });
  return result;
}
...
function looksLikeHeaders(row) {
  const actual = Array.isArray(row) ? row.map(x => String(x == null ? '' : x).trim()) : [];
  return actual.indexOf('id') === 0;
}
