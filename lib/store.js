...
async function deleteSong(id) {
  ...
  const likes = await db.query('SELECT id FROM likes WHERE song_id = $1', [songId]);
  await db.query('DELETE FROM votes WHERE song_id = $1', [songId]);
  await db.query('DELETE FROM likes WHERE song_id = $1', [songId]);
  await db.query('DELETE FROM lineup WHERE song_id = $1', [songId]);
...
async function claimVote(device, songId) {
  ...
  const result = await db.query(
    `INSERT INTO votes (device, song_id, created_at, like_id) VALUES ($1, $2, $3, '')
     ON CONFLICT (device, song_id) DO NOTHING RETURNING device`,
