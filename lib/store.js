...
function updateRow(key, id, patch, options) {
...
function listVotes(device) {
  const id = String(device == null ? '' : device);
  if (id === '') {
    return new Set();
  }
  const rows = db.getDatabase().prepare('SELECT song_id FROM votes WHERE device = ?').all(id);
  return new Set(rows.map(x => x.song_id));
}

function hasVote(device, songId) {
  const id = String(device == null ? '' : device);
  if (id === '') {
    return false;
  }
  const row = db
    .getDatabase()
    .prepare('SELECT 1 AS found FROM votes WHERE device = ? AND song_id = ?')
    .get(id, String(songId));
  return row != null;
}

function addVote(device, songId) {
  const id = String(device == null ? '' : device);
  if (id === '') {
    return;
  }
  db.getDatabase()
    .prepare('INSERT INTO votes (device, song_id, created_at) VALUES (?, ?, ?) ON CONFLICT (device, song_id) DO NOTHING')
    .run(id, String(songId), new Date().toISOString());
}
