...
async function addLikeFrom(device, songId, name) {
  const claimed = await store.claimVote(device, songId);
  if (!claimed) {
    return null;
  }
  const like = await addLike(songId, name);
  await store.attachLike(device, songId, like.id);
  return like;
}
